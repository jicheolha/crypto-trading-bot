        ...
        try:
            ...
            for account in accounts:
                ...
                if total_balance > 0:
                    if currency in ['USD', 'USDC']:
                        ...
                        logger.info(f"  ðŸ’µ {currency}: ${total_balance:,.2f} (available: ${avail:,.2f})")
                    else:
            ...
            if spot_positions:
                ...
                for currency, amount, price, value in spot_positions:
                    logger.info(f"  ðŸª™ {currency}: {amount:.8f} @ ${price:,.2f} = ${value:,.2f}")
            ...
            if hasattr(order, 'success') and not order.success:
                ...
                if hasattr(order, 'error_response'):
                    ...
                    if preview_reason == 'PREVIEW_INSUFFICIENT_FUNDS_FOR_FUTURES':
                        ...
                        logger.error(f"   Transfer funds: Coinbase â†’ Transfer â†’ Spot to Futures")
                    else:
    ...
    def check_entries(self):
        ...
        for trade_symbol in self.symbols:
            ...
            if is_new_candle:
                ...
                if len(df) >= 2:
                    ...
                    if prev['Squeeze'] and not current['Squeeze']:
                        logger.info(f"   ðŸŽ¯ SQUEEZE RELEASED after {prev['Squeeze_Duration']:.0f} bars!")
                    elif current['Squeeze']:
                        logger.info(f"   â³ Waiting for squeeze release...")
            ...
            if signal.direction != 'neutral':
                logger.info(f"   âœ“ SIGNAL: {signal.direction.upper()}")
