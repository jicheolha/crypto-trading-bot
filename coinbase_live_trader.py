...
class CoinbaseLiveTrader:
    ...
    def _detect_product_types(self):
        ...
        for symbol in self.symbols:
            try:
                ...
                if is_futures:
                    # Try to get contract size from product
                    if hasattr(product, 'contract_size'):
                        contract_size = float(product.contract_size)
                    else:
                        # Default for Coinbase ETH futures
                        contract_size = 0.1
                    
                    # Leverage - try to get from product, else default
                    if hasattr(product, 'leverage'):
                        leverage = float(product.leverage)
                    else:
                        # Default for Coinbase futures (typically 2-5x)
                        leverage = 4.0
        ...
        try:
            ...
            if spot_positions:
                ...
                for currency, amount, price, value in spot_positions:
                    logger.info(f"  🪙 {currency}: {amount:.8f} @ ${price:,.2f} = ${value:,.2f}")
            ...
            logger.info("")
            logger.info("="*60)
            logger.info(f"Trading Capital (USD/USDC): ${total_usd:,.2f}")
            if total_crypto_value > 0:
                logger.info(f"Crypto Holdings (not trading): ${total_crypto_value:,.2f}")
                logger.info(f"Total Portfolio Value: ${total_usd + total_crypto_value:,.2f}")
            logger.info("="*60)
            ...
            if hasattr(order, 'success') and not order.success:
                ...
                if hasattr(order, 'error_response'):
                    ...
                    if preview_reason == 'PREVIEW_INSUFFICIENT_FUNDS_FOR_FUTURES':
                        ...
                        logger.error(f"   Your ${self.get_balance():.2f} is in SPOT account (CBI)")
                        logger.error(f"   Futures trade on FUTURES account (CFM)")
    ...
    def check_entries(self):
        ...
        for trade_symbol in self.symbols:
            ...
            # Check for new candle on signal symbol
            latest = signal_df.index[-1]
            candle_key = f"{signal_symbol}_{self.signal_timeframe}"
            if self.last_candles.get(candle_key) == latest:
                continue
            
            self.last_candles[candle_key] = latest
            
            # Log new candle
            logger.info(f"New {self.signal_timeframe} candle at {latest}")
            
            # Calculate indicators for diagnostics
            df = self.analyzer.calculate_indicators(signal_df.tail(100))
            ...
            if len(df) >= 2:
                ...
                if prev['Squeeze'] and not current['Squeeze']:
                    ...
                    logger.info(f"   ⏳ Waiting for squeeze release...")
            
            # Generate signal from SIGNAL symbol data
            self.signal_generator.set_signal_data({signal_symbol: signal_df})
            self.signal_generator.set_atr_data({signal_symbol: atr_df})
            
            signal = self.signal_generator.generate_signal(
                signal_df,
                signal_symbol,  # Generate for signal symbol
            ...
            if signal.direction != 'neutral':
                ...
                # BUT execute on TRADE symbol (e.g., ETP-20DEC30-CDE futures)
                signal.symbol = trade_symbol  # Change symbol to trade symbol
                ...
                if len(df) >= 2:
                    ...
                    logger.info(f"   No signal: Insufficient data")
