"""
Backtester for Bollinger Bands Squeeze Breakout Strategy
"""
import logging
...
class Position:
    ...
    atr: float
    reasons: List[str]
    entry_equity: float = 0.0  # Total equity at entry
    position_size_pct: float = 0.0  # Position size as % of equity
    risk_amount: float = 0.0  # $ risk (distance to stop)
    risk_pct: float = 0.0  # % risk of equity
    r_multiple: float = 0.0  # P&L / risk (reward:risk ratio)
    pnl: float = 0.0
    pnl_pct: float = 0.0
...
class BBSqueezeBacktester:
    ...
    def __init__(
        ...
        max_hold_days: float = None,  # Max days to hold a position (None = no limit)
    ...
    def run_backtest(self, data: Dict[str, pd.DataFrame]) -> BacktestResults:
        ...
        for symbol in list(self.positions.keys()):
            if symbol in data:
                ...
                self._close(symbol, final_price, all_times[-1], "End of backtest", self.capital)
    ...
    def _calculate_stats(self, equity: pd.Series) -> Dict:
        ...
        avg_win_r = avg_r_win if avg_r_win else 1
        avg_loss_r = abs(avg_r_loss) if avg_r_loss else 1
        kelly = (win_rate / avg_loss_r) - ((1 - win_rate) / avg_win_r) if avg_loss_r > 0 and avg_win_r > 0 else 0
