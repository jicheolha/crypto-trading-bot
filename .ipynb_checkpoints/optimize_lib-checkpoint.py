"""
Squeeze Breakout Strategy Optimizer (LONG-ONLY)

Optimized for Alpaca crypto trading (no shorting support).

Methods:
1. Random Search - Fast exploration
2. Bayesian/TPE (Optuna) - Smart optimization
...
Usage:
    ...
    python optimize.py --method grid  # Warning: slow!
"""
import argparse
import logging
import warnings
...
# Long only mode (Alpaca crypto doesn't support shorting)
LONG_ONLY = True
...
PARAM_SPACE = {
    ...
    # RSI (for long-only, we only care about overbought filter)
    'rsi_period': {
        'type': 'int',
        'low': 7,
        'high': 21,
...
def run_backtest(data: Dict, params: Dict[str, Any]) -> Tuple[float, Dict]:
    ...
    signal_gen = BBSqueezeSignalGenerator(
        ...
        rsi_oversold=20,  # Fixed low value for long-only (not blocking shorts)
...
class BayesianOptimizer(BaseOptimizer):
    ...
    def __init__(self, data: Dict, train_ratio: float = 0.7, n_startup: int = 30):
    ...
    def optimize(self, n_trials: int) -> OptimizationResult:
        ...
        print(f"{'='*60}")
        print(f"Trials: {n_trials} | Startup (random): {self.n_startup}")
...
class MultiStageOptimizer:
    ...
    def optimize(
        ...
        bayesian_opt = BayesianOptimizer(self.data, self.train_ratio, n_startup=20)
...
# RSI (long-only: only overbought matters)
RSI_PERIOD = {params['rsi_period']}
RSI_OVERBOUGHT = {params['rsi_overbought']}
...
def main():
    ...
    if args.method == 'random':
        ...
        optimizer = BayesianOptimizer(data, n_startup=min(50, args.trials // 4))
        result = optimizer.optimize(args.trials)
